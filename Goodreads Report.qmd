...
# more dynamic way of determining last book read
last.book = reads %>% slice_head(n=1)

last.title = as.character(last.book$title)
last.author = as.character(last.book$author)
last.date = as.character(format(as.Date(last.book$date_read), "%d %B %Y"))
```
...
```{r}
reads %>%
  select(title_trunc_60, my_review, my_rating) %>%
  filter(!is.na(my_review)) %>%
  slice_sample(n=1) %>%
  gt() %>%
  cols_label(title_trunc_60 = "Title", 
             my_review = "Review",
             my_rating = "Rating")
```
...
reads %>% 
  ...
  slice_max(order_by = number_of_pages, n = 7, with_ties = T) %>%
  gt() %>%
  fmt_integer() %>%
  cols_label(
    title_trunc = "Title",
...
reads %>% 
  filter(my_rating!=0) %>%
  select(title_trunc, author, number_of_pages) %>%
  slice_min(order_by = number_of_pages, n = 7, with_ties = T) %>%
  gt() %>%
  fmt_integer() %>%
  cols_label(
    title_trunc = "Title",
